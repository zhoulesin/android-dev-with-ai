...
如果你最近打开 GitHub 上任何一个 AI 工具的开源项目——Superpowers、Cline、Roo Code、Cline DoneRight——你会发现 Skills 目录正在疯狂膨胀。有的仓库已经有 50+ 个 Skills，覆盖从"写 SQL"到"审查 GDPR 合规"的一切。
...
如果你不想自己从头建 Android Skills，以下社区资源值得看看：

- **Superpowers Compose Rules**：GitHub 上能找到的一些 Compose 最佳实践的规则集
- **Android Security Rules**：建议基于 OWASP MASVS 提取关键检查项，但要注意别一刀切——不是所有数据存储都需要加密
- **Gradle Dependency Rules**：在 CLAUDE.md 中声明使用 version catalog (`libs.versions.toml`)，禁止模块间直接 `implementation` 传递依赖
